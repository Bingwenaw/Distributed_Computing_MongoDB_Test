    ...
    def execute(self, operation, settings, database="lab", collection="manual",
        ...
        with self.lock:
            ...
            context = dict(operation_id=op_id, client_sequence=self.sequence, trial=trial,
                           cluster=self.cluster.name, laptop=self.cluster.laptop)
            ...
            try:
                if settings.route not in self.cluster.routes:
                    raise ValueError("Read target is not part of the active cluster.")
...
def topology(cluster=LOCAL):
    """Probe every configured member, including unreachable ones."""
    def probe(item):
        name, (host, port) = item
        try:
            with MongoClient(f"mongodb://{host}:{port}/?directConnection=true",
                ...
                if hello.get("setName") != cluster.name:
                    raise ValueError("Node is not in the expected replica set.")
                return dict(node=name, laptop=cluster.owner(name), endpoint=f"{host}:{port}", reachable=True,
            ...
            return dict(node=name, laptop=cluster.owner(name), endpoint=f"{host}:{port}", reachable=False, role="UNREACHABLE", error=str(exc))
    with ThreadPoolExecutor(max_workers=len(cluster.nodes)) as pool:
        return list(pool.map(probe, cluster.nodes.items()))
