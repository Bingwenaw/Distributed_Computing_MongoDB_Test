...
from lab import PROPERTIES, ROOT, Settings, prediction, topology
...
def wait_topology(predicate, log, cancel, seconds=35, cluster=LOCAL):
...
def healthy(state, cluster=LOCAL):
    return {n["node"] for n in state} == set(cluster.nodes) and sum(n["role"] == "PRIMARY" for n in state) == 1 and sum(n["role"] == "SECONDARY" for n in state) == len(state) - 1


def apply_scenario(scenario, log, cancel, cluster=LOCAL):
    if cluster.linked and scenario != "Normal":
        raise ValueError("Shared automated trials support Normal only; owners control faults manually.")
    before = topology(cluster)
    if not healthy(before, cluster):
        raise Inconclusive("Trial requires every configured node healthy before fault injection.")
    ...
    for node in targets:
        change("isolate" if scenario == "Network partition" else "stop", node, log, cluster)
    def achieved(state):
    ...
    after = wait_topology(achieved, log, cancel, cluster=cluster)
...
def trial(actors, log, settings, property_name, scenario, cancel):
    ...
    def sample(phase):
        ...
        nodes = [n["node"] for n in topology(cluster) if n["role"] in ("PRIMARY", "SECONDARY")]
        if not nodes:
    ...
    try:
        wait_topology(lambda state: healthy(state, cluster), log, cancel, cluster=cluster)
        ...
        apply_scenario(scenario, log, cancel, cluster)
        ...
        if fault_attempted:
            try:
                ...
                wait_topology(lambda state: healthy(state, cluster), log, threading.Event(), cluster=cluster)
...
def run_suite(actors, log, settings, properties, scenarios, repetitions, matrix, cancel):
    cluster = actors[0].cluster
    if cluster.linked and any(s != "Normal" for s in scenarios):
        raise ValueError("Shared automated trials support Normal only.")
    ...
    try:
        for cfg, prop, scenario, repetition in product(configs, properties, scenarios, range(repetitions)):
            ...
            if results[-1]["evidence"]["errors"] and not healthy(topology(cluster), cluster):
